// ============================================================
// TRACK — geometry calculation and canvas rendering
// ============================================================
...
import {
  ...
  SEMICIRCLE_RADIUS,
  STRAIGHT_LENGTH,
  TRACK_INNER_RAIL_OFFSET,
  TRACK_OUTER_RAIL_OFFSET,
  DISTANCE_MARKER_POSITIONS,
  TOTAL_QUESTIONS,
  PALETTE,
} from '../core/config.js';
...
const SR = SEMICIRCLE_RADIUS;
const SL = STRAIGHT_LENGTH;
const PERIM = 2 * SL + 2 * Math.PI * SR;

let trackTime = 0;

/**
 * Get point on track centerline at position t (0-1), with lane offset.
 * laneOffset > 0 = outside, < 0 = inside.
 * Track runs CLOCKWISE: top-straight L->R, right curve, bottom R->L, left curve.
 */
export function trackPoint(t, laneOffset) {
  t = ((t % 1) + 1) % 1;
  const d = t * PERIM;
  let x, y, nx, ny, angle;
...
export function drawTrack(ctx, elapsed, running) {
  ...
  grassGrad.addColorStop(0, PALETTE.grassBright);
  grassGrad.addColorStop(0.5, PALETTE.grass);
  grassGrad.addColorStop(1, PALETTE.grassDark);
  ...
  // Dirt track surface
  ctx.lineWidth = 56;
  ctx.strokeStyle = PALETTE.trackDirtDark;
  ctx.beginPath();
  for (let i = 0; i <= 200; i++) {
    const pt = trackPoint(i / 200, 9);
  ...
  ctx.lineWidth = 54;
  ctx.strokeStyle = PALETTE.trackDirt;
  ctx.beginPath();
  for (let i = 0; i <= 200; i++) {
    const pt = trackPoint(i / 200, 9);
    if (i === 0) ctx.moveTo(pt.x, pt.y);
  ...
  // Lane lines (dynamic based on horse count — default 3)
  const laneCount = 3;
  for (let lane = 0; lane < laneCount; lane++) {
    ...
    for (let li = 0; li <= 200; li++) {
      const lpt = trackPoint(li / 200, loff);
  ...
  // Rails
  [TRACK_INNER_RAIL_OFFSET, TRACK_OUTER_RAIL_OFFSET].forEach((offset, ri) => {
    ctx.strokeStyle = ri === 0 ? 'rgba(196,164,74,0.08)' : 'rgba(196,164,74,0.05)';
    ctx.lineWidth = 6;
    ctx.beginPath();
    for (let j = 0; j <= 200; j++) {
      const pt = trackPoint(j / 200, offset);
    ...
    for (let rp = 0; rp < 24; rp++) {
      const rpt = trackPoint(rp / 24, offset);
  ...
  // Distance markers
  DISTANCE_MARKER_POSITIONS.forEach((m) => {
    const mpt = trackPoint(m, 35);
    ctx.font = '500 9px "JetBrains Mono", monospace';
    ctx.fillStyle = 'rgba(255,255,255,0.15)';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(Math.round(m * 100) + '%', mpt.x, mpt.y);
    const mpt2 = trackPoint(m, 31);
    ctx.fillStyle = 'rgba(196,164,74,0.3)';
    ctx.fillRect(mpt2.x - 1, mpt2.y - 4, 2, 8);
  });

  // Finish line
  const flStart = trackPoint(0, TRACK_INNER_RAIL_OFFSET);
  const flEnd = trackPoint(0, TRACK_OUTER_RAIL_OFFSET);
  ...
  ctx.fillText(`MMLU-Pro Hard \u00b7 ${TOTAL_QUESTIONS} Questions`, TX + SR + SL / 2, TY + SR + 15);
